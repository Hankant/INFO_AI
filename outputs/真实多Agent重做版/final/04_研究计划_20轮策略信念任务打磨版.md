...
**关键词：** AI delegation; algorithm aversion; repeated decision game; beauty contest; EWA; role learning; human-AI collaboration; asymmetric learning
...
问题备忘录：
20 轮到底够不够？
20 轮比 40 轮更可行，也比 8–12 轮更能覆盖学习过程；但它是否足以支持 “persistent post-error under-delegation” 仍然需要谨慎表述。现在最好不要说长期稳态，只说有限重复窗口中的持续性低授权。你的 RP 现在已经采用这个更稳的说法。
选美博弈会不会太快被学会？
...
AI 与 human analyst 的表现等价能不能真的做到？
这是实验识别的命门。RP 现在已经写了胜率、平均绝对误差、错误严重程度和完全委托收益四重等价控制，这很好。争议在于：20 轮很短，配平空间有限，所以必须做一个轮次矩阵，否则读者会怀疑等价控制只是原则声明。
EWA 会不会在 20 轮里估不稳？
这是方法上最大的风险。20 轮可以用 EWA-inspired model，但不要把它当强结构估计。主证据应该是 reduced-form 条件比较，EWA 只作为机制辅助，重点检验 |β_AI−| > |β_H−|，不要估太多复杂参数。
五种协作方式是否足够表达“角色学习”？
